...
library(ggtext)#text rendering for ggplot2
library(here)
library(glue) #string 
library(systemfonts)
library(pdftools)#extracting text, fonts; rendering of PDF, PNG
library(patchwork) #extending ggplots in panels


theme_set(theme_void(base_family = "Roboto Condensed")) #uses Roboto Condensed font

theme_update(
  axis.text.x = element_text(size = 9, color = "grey25", vjust = 1,
                             margin = margin(t = -10)),
  #axis.text.x customzies font size, color, etc
...
df_employed_2020 <-
  df_employed %>% 
  filter(year == 2020, 
         !is.na(industry), #select all non-NA values and remove values below
         !industry %in% c("Men", "Women", 
...
df_employed_2020_total <-
  df_employed_2020 %>% #take df_employed_2020 then filter by 'Total' in race_gender variable, then group_by industry to summarize employ_n count by industry. 
  filter(race_gender == "TOTAL") %>% 
  group_by(industry) %>% 
  summarize(employ_n = sum(employ_n, na.rm = TRUE)) %>% 
  mutate(total = sum(employ_n)) %>% #mutate total to compute perc
  group_by(industry) %>% 
  mutate(perc = employ_n / total) %>% 
  ungroup() %>% #ungroup previous grouping
  arrange(-employ_n) %>% 
  add_row(industry = "SUM") %>% 
  mutate(
    industry = fct_reorder(industry, -employ_n),
    rank = row_number()
  )#factor levels reorder; this is useful function for bar graphs
```
...
k_plot_div <- function(group, pos = .6, annotation) {
  df <- df_employed_2020 %>% 
    ...
    group_by(industry) %>% 
    mutate(perc = employ_n / total) %>% 
    ungroup() %>% #employ stand, em
    mutate(
      employ_stand = employ_n / max(employ_n),
      employ_lab = case_when(
        employ_n > 1000000 ~ 
          paste0(format(employ_n / 1000000, 
                        big.mark = ",", 
                        digits = 1, 
                        trim = TRUE), 
                 "M"),
        employ_n > 1000 ~ 
          paste0(format(employ_n / 1000, 
                        big.mark = ",", 
                        trim = TRUE), 
                 "K"),
    ...
    arrange(rank) %>% #use mutate below for min and max for x- and y-axes 
    mutate(
      lag = lag(perc), #from percentage variable above
      end = cumsum(perc), #cumulative sums of variables
      end = if_else(!is.na(lag), 
                    end, 
                    perc), #if lag values don't exist, put end, otherwise perc 
      start = end - perc,
      img = glue("{here()}/img/industries/{industry}.png")
    ) %>% #save series of files
    add_row(
      industry = "SUM", 
      rank = 17,
...
for(i in 1:25) {
  ...
  ggsave(here("plots", 
              "2021_09", 
              glue("2021_09_Employment_Total_{i}.pdf")),
         width = 16, 
         height = 11.2, 
         device = cairo_pdf)
}
...
## loop to find combination since ggrepel segments are sometimes too long
## even with fixed xlim and seed — no clue why ¯\_(ツ)_/¯
##remove theme_title functions for the code to work
for(i in 1:25) {
